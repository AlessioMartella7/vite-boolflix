      ...
      <div class="card h-100 rounded-0" >
        <div class="card-body bg-black d-flex flex-column p-0 ">
          <div class="card-info text-white position-absolute top-0 start-0 end-0 bottom-0 justify-content-start align-items-start flex-column p-3">
              <!-- titolo film -->
               <h4>Titolo: <span class="fs-6">{{ movie.title }}</span></h4>
            
              <!-- titolo originale film -->
               <h4>Titolo Originale: <span class="fs-6">{{ movie.original_title }}</span></h4>
              
              <!-- lingua originale  -->
              <img class="lang-flag mb-2" :src="getFlagImg(movie.original_language)" :alt="movie.original_language">

              <!-- voto medio stelle -->
              <div class="d-flex">
                <h4>Voto:<font-awesome-icon v-for="star in transformToInt(movie.vote_average)" icon="star" class="star text-warning fs-2"/>
                </h4>
              </div>

               <!-- Overview -->
               <h4>Overview: <span class="fs-6"> {{ movie.overview }}</span></h4>
